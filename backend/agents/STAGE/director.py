...
def format_agents_with_personas(agents: List[Agent]) -> str:
    """Format agents with their personas for the Director prompt."""
    lines = []
    for a in agents:
        if a.persona:
            lines.append(f"- **{a.name}**: {a.persona}")
        else:
            lines.append(f"- **{a.name}**: (no persona defined)")
    return "\n".join(lines)
...
def build_director_user_prompt(treatment: str, messages: List[Message], agents: List[Agent], human_user: str = "participant", chatroom_context: str = "") -> str:
    ...
    agents_formatted = format_agents_with_personas(agents)
    ...
    # Append the list of available agents with their personas
    prompt += f"\n\n## Available Agents\n\nEach agent has a distinct personality. You MUST ensure each agent behaves consistently with their persona throughout the conversation:\n\n{agents_formatted}\n\n**IMPORTANT**: Maintain consistency - each agent should stay true to their personality traits. Do not make agents contradict their established character or previous statements.\n"
